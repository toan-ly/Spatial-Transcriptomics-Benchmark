...
def main():
    ...
    adata, Ann_df = load_colon_visium_hd(dir_input, spatial_dir) 
    ...
    for seed in SEEDS:
        ...
        dir_out = output_path / f'{seed}/STAGATE'
        ...
        STAGATE.Cal_Spatial_Net(adata, model='KNN', k_cutoff=6)
        STAGATE.Stats_Spatial_Net(adata)

        ## Runing STAGATE
        adata = STAGATE.train_STAGATE(adata, device=device, random_seed=seed)
        print(f'Finish training STAGATE for {sample_name}')
        ...
        low_dim_data.to_csv(dir_out / 'low_dim_data.csv')
        # expression_data.T.to_csv(f"{dir_out}/expression_matrix.csv")
        cell_metadata.to_csv(dir_out / 'cell_metadata.csv')
        ...
        umap_df.to_csv(dir_out / 'spatial_umap_coords.csv', index=False)
